...
class Mango_Loworderfee_Model_Observer extends Mage_Core_Model_Abstract {
    ...
    public function sales_quote_collect_totals_after(Varien_Event_Observer $observer) {
		...
		$amount = Mage::getStoreConfig('sales/minimum_order/low_order_fee', $store->getId());
            $address->setGrandTotal($address->getGrandTotal() + $address->getTaxAmount() + $amount);
            $address->setBaseGrandTotal($address->getBaseGrandTotal() + $address->getBaseTaxAmount() + $amount);
